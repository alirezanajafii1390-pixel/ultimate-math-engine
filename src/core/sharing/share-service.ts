  ...
  const share = useCallback(
    async (data: ShareData, deepLink?: string): Promise<ShareOutcome> => {
      ...
      try {
        ...
        // Telegram's native picker (shareURL) needs an actual URL — only
        // used when the caller supplied one (all three pages do, though a
        // very long Formula/Converter/Calculator state can still fail to
        // fit Telegram's 64-char start_param limit and arrive as
        // undefined — see platform/telegram/config.ts). Falls through to
        // Web Share below otherwise, same as outside Telegram — still a
        // proper native share sheet, just not Telegram's own in-chat
        // picker.
