...
export default function TelegramPlatformProvider({ children }: { children: ReactNode }) {
  ...
  // Instance-scoped (not module-scoped): guards against React StrictMode's
  // double-invoke within a single mount, without any risk of surviving
  // into a genuinely fresh mount of this component — e.g. after Telegram's
  // in-app "Reload Page" action, if that turns out to re-mount the React
  // tree without a full document navigation (soft reload). A module-level
  // `let` here previously could have left backButton/mainButton wired to
  // a stale bridge session in that case — the classic symptom being a
  // frozen Back Button that stops responding after using Reload Page.
  const sdkInitStarted = useRef(false);

  useEffect(() => {
    if (sdkInitStarted.current) {
      setInitialized(true);
      return;
    }
    sdkInitStarted.current = true;
    try {
      init();
      ...
      viewport
        ...
        .then(() => {
          ...
          // called eagerly above. Not consumed by our own CSS today (the
          // App Shell layout contract in index.css — --app-header-height/
          // --app-bottom-nav-inset/--app-safe-* — replaced the one place
          // that used to need this, see PROJECT_CONTEXT.md); left mounted
          // as general-purpose infrastructure for anything that later
          // genuinely needs Telegram's raw viewport pixels.
      ...
      setInitErrorMessage(e instanceof Error ? `${e.name}: ${e.message}` : String(e));
      setInitFailed(true);
    }
  }, []);
  ...
  const adapter: PlatformAdapter = useMemo(
    ...
    [initFailed, initErrorMessage, colorScheme, safeAreaInsets, contentSafeAreaInsets, isBackButtonSupported, isMainButtonSupported, isCloudStorageSupported, isFullscreen, isHapticsSupported, rawInitData, user, startParam],
  );
