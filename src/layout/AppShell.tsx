...
import { decodeDeepLink } from '../core/deeplink';
...
export default function AppShell() {
  ...
  // mobile browser's would be — so inside Telegram we override the
  // central --app-safe-* contract (see index.css) directly. Outside
  // Telegram, :root's own default (real env()) already applies with no
  // code needed here at all — this effect only exists to override it.
  useEffect(() => {
  ...
  // Measures the floating bottom nav's REAL occupied footprint (distance
  // from the viewport's bottom edge to the nav's own top edge) and
  // exposes it as --app-bottom-nav-inset (see index.css) — the one
  // dimension in the Shell's layout contract that can't be a fixed
  // constant, since it's an intentional floating overlay whose size
  // depends on rendered padding/content, not a formula. Content (most
  // importantly Calculator) reads this instead of guessing; nothing here
  // is platform-specific — it's just "how tall is this element, really".
  // Re-measures on resize/orientation change and whenever the nav's own
  // box changes (safe-area updates, font-size changes, etc.) via
  // ResizeObserver — no viewport value is ever assumed static.
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      // A hidden element (e.g. md:hidden on desktop) reports an all-zero
      // rect — without this check, that would compute a bogus inset equal
      // to the entire window height instead of correctly meaning "the nav
      // occupies no space right now".
      const isHidden = rect.width === 0 && rect.height === 0;
      const inset = isHidden ? 0 : Math.max(0, window.innerHeight - rect.top);
      document.documentElement.style.setProperty('--app-bottom-nav-inset', `${inset}px`);
    };
    measure();
    ...
    return () => {
      ...
      document.documentElement.style.setProperty('--app-bottom-nav-inset', '0px');
    };
  }, []);
  ...
  // that logic here. Tries the unified multi-section codec first; falls
  // back to the original calculator-only one so links shared before this
  // still work. Once only, on launch — a ref guard rather than an empty
  // dep array alone, since `platform` itself changes reference as
  ...
  useEffect(() => {
    ...
    const payload = decodeDeepLink(raw);
    if (payload) {
      if (payload.kind === 'calculator') {
        navigate(`/calculator?expr=${encodeURIComponent(payload.expr)}`, { replace: true });
      } else if (payload.kind === 'formula') {
        const varParams = Object.entries(payload.values)
          .map(([k, v]) => `v_${encodeURIComponent(k)}=${encodeURIComponent(v)}`)
          .join('&');
        navigate(`/formula?f=${encodeURIComponent(payload.id)}${varParams ? `&${varParams}` : ''}`, { replace: true });
      } else if (payload.kind === 'converter') {
        navigate(
          `/converter?cat=${encodeURIComponent(payload.cat)}&from=${encodeURIComponent(payload.from)}&to=${encodeURIComponent(payload.to)}&val=${encodeURIComponent(payload.value)}`,
          { replace: true },
        );
      }
      return;
    ...
    // Backward-compat fallback: a link encoded before the unified codec
    // existed, when Calculator was the only section with deep links.
  ...
  return (
    <div className="relative h-full">
      ...
      <header className="nav-chrome glass fixed inset-x-0 top-0 z-30 flex h-[var(--app-header-height)] items-center justify-between px-3.5 md:hidden">
      ...
      {/* ── Content ── */}
      <main className="relative z-10 flex h-dvh flex-col md:ps-[232px]">
        <div className="min-h-0 flex-1 overflow-y-auto pb-[var(--app-bottom-nav-inset)] pt-[var(--app-header-height)] md:pb-0 md:pt-0">
      ...
      <nav ref={bottomNavRef} className="nav-chrome fixed inset-x-3 z-30 md:hidden" style={{ bottom: 'calc(6px + var(--app-safe-bottom))' }}>
      ...
      {state.focusMode && (
        <button
          onClick={toggleFocus}
          className="glass-strong press focus-ring fixed end-3 top-[max(0.75rem,var(--app-safe-top))] z-40 flex h-10 items-center gap-2 rounded-full px-4 text-sm font-medium text-[var(--accent-highlight)] shadow-[var(--shadow-2)]"
