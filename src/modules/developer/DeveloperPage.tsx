...
export default function DeveloperPage() {
  ...
  return (
    <Page>
      ...
      {/* Changelog — reads directly from CHANGELOG.md (Vite ?raw import),
          so this is never out of sync with the actual file. */}
      <Section title="Changelog">
        <Card className="max-h-96 overflow-auto p-4">
          <pre className="whitespace-pre-wrap text-xs leading-6 text-[var(--text-secondary)]">{changelogRaw}</pre>
        </Card>
      </Section>
    </Page>
  );
