...
export default function ConverterPage() {
  ...
  useEffect(() => {
    ...
    if (c && getCategory(c)) {
      ...
      const newFrom = from && list.some((u) => u.id === from) ? from : (list[0]?.id ?? '');
      const newTo = to && list.some((u) => u.id === to) ? to : (list[1]?.id ?? list[0]?.id ?? '');
      setFromId(newFrom);
      setToId(newTo);
      if (val !== null) setValue(val);
      lastSig.current = `${c}:${newFrom}:${newTo}:${val ?? value}`;
    }
  ...
  const shareConversion = useCallback(() => {
    ...
    const deepLink = buildAppDeepLink(encodeDeepLink({ kind: 'converter', cat: cat.id, from: fromId, to: toId, value }));
    shareViaService(
      {
        kind: 'converter',
  ...
  }, [result, fromUnit, toUnit, cat, fromId, toId, value, lang, num, state.settings.precision, shareViaService]);
          ...
          <>
            <div className="mb-3 flex justify-end">
              <Btn
                ...
                onClick={() => {
                  haptics.destructive();
                  dispatch({ type: 'history:clear', kind: 'converter' });
                }}
              >
                <Trash2 size={14} />
                {t('calc.clearAll')}
