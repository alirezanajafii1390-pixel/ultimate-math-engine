...
import { buildAppBaseLink, buildAppDeepLink } from '../../platform/telegram/config';
import { encodeDeepLink } from '../../core/deeplink';
    ...
    <Page>
      ...
      <div className="chip-row mb-4 pb-1">
        {tabs.map((tb) => (
          <button
            key={tb.key}
            className={`chip press focus-ring shrink-0 ${tab === tb.key ? 'active' : ''}`}
            onClick={() => {
              haptics.toggle();
              setTab(tb.key);
            }}
      ...
      {tab === 'all' && (
        <div className="chip-row mb-5 pb-1">
          <button
            className={`chip press focus-ring shrink-0 ${cat === 'all' ? 'active' : ''}`}
            onClick={() => {
              haptics.toggle();
              setCat('all');
            }}
          ...
          {FORMULA_CATEGORIES.map((c) => (
            <button
              key={c.id}
              className={`chip press focus-ring shrink-0 ${cat === c.id ? 'active' : ''}`}
              onClick={() => {
                haptics.toggle();
                setCat(c.id);
              }}
          ...
          <>
            <div className="mb-3 flex justify-end">
              <Btn
                ...
                onClick={() => {
                  haptics.destructive();
                  dispatch({ type: 'history:clear', kind: 'formula' });
                }}
              >
                {t('calc.clearAll')}
              </Btn>
...
function FormulaDetail({ f, onBack }: { f: FormulaDef; onBack: () => void }) {
  ...
  const [values, setValues] = useState<Record<string, string>>(() => {
    ...
    for (const v of f.vars) {
      const fromLink = searchParams.get(`v_${v.key}`);
      if (fromLink !== null) initial[v.key] = fromLink;
    }
    return initial;
  });
  ...
  const toggleFav = useCallback(() => {
    ...
    haptics.toggle();
    toast(fav ? t('toast.removedFav') : t('toast.addedFav'), 'info');
  }, [dispatch, f.id, fav, toast, t, haptics]);
  ...
  const shareFormulaResult = useCallback(() => {
    ...
    const deepLink = buildAppDeepLink(encodeDeepLink({ kind: 'formula', id: f.id, values }));
    shareViaService(
      {
        kind: 'formula',
      ...
      deepLink ?? buildAppBaseLink() ?? undefined,
    );
  }, [displayResult, f, values, numeric, state.settings.precision, displayUnitSymbol, lang, shareViaService, t]);
